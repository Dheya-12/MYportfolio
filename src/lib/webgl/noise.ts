...
export function turbulence(
  ...
  for (let i = 0; i < config.octaves; i++) {
    const noiseValue = Math.abs(perlinNoise2D(x * frequency, y * frequency));
    value += noiseValue * amplitude;
...
export const BREATHING_PRESET: NoiseConfig = {
  frequency: 1.5,
  amplitude: 0.3,
  octaves: 3,
  persistence: 0.5,
...
export const WAVE_PRESET: NoiseConfig = {
  frequency: 1.0,
  amplitude: 0.6,
  octaves: 4,
  persistence: 0.6,
...
export const DETAIL_PRESET: NoiseConfig = {
  frequency: 4.0,
  amplitude: 0.2,
  octaves: 5,
  persistence: 0.4,
